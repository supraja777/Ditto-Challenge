...
def updating_config_values(total_matches, successful_matches, target_matches=5):
    ...
    if success_rate < 0.5 and total_matches > 0:
        ...
        # Reduce the weight of "hard" facts like age 
    ...
    # If AI date simulations (judge) aren't predicting success well we will rely more on math (normal)
    if success_rate < 0.4:
    ...
    if success_rate < 0.3:
        # High failure - Increase exploration to find new patterns
        ...
        # High success - Continue current winning formula (exploitation)
